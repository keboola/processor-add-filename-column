...
export REPOSITORY=`docker run --rm  \
    ...
    quay.io/keboola/developer-portal-cli-v2:latest ecr:get-repository keboola processor-add-filename-column`
...
eval $(docker run --rm \
    ...
    quay.io/keboola/developer-portal-cli-v2:latest ecr:get-login keboola processor-add-filename-column)
...
docker run --rm -e KBC_DEVELOPERPORTAL_USERNAME -e KBC_DEVELOPERPORTAL_PASSWORD -e KBC_DEVELOPERPORTAL_URL \
    quay.io/keboola/developer-portal-cli-v2:latest update-app-repository keboola processor-add-filename-column ${TRAVIS_TAG}
